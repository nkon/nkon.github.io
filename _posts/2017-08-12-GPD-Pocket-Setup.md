...
* Intel Dynamic Platform and Thermal Framework(DPTF) ドライバのアップデート。標準では8.1系が入っているはずだ。デバイスマネージャからのアップデートでは更新されないが、Windows　Update Catalog のページからドライバを検索＆ダウンロード＆cab を展開して、*.inf を右クリックして個別にドライバをインストールすればバージョンアップできる。8.2系にしたら、だいぶトラブルが減った。
* BIOS は 8/7 版と呼ばれているもの、または最新版にアップデートする。ダウンロードして、展開して、`update_win./bat`というバッチファイルを管理者権限で実行する。　できれば、満充電の電池動作状態がよいだろう。途中で再起動するとハマるので、きちんと更新できるまでちゃんと待つこと。いくつかのセクションに書き込んでいるが、それほど時間はかからないはずだ。
