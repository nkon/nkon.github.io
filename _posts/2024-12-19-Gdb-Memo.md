...
今は違うかもしれないが、Apple Silicone のMacでhome-brewを使ってインストールしたとき、このページにあるバグに遭遇したので、[`arm-none-eabi-gdb`](https://formulae.brew.sh/formula/arm-none-eabi-gdb)ではなく[`gcc-arm-embedded`](https://formulae.brew.sh/cask/gcc-arm-embedded)を使っている。ただし、`gcc-arm-embedded`のgdbはpythonが組み込まれていないので[`gdb-dashboard`](https://github.com/cyrus-and/gdb-dashboard)は使えない。
