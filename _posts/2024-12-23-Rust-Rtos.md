...
<!-- Added by: nkon, at: 2024年 12月23日 月曜日 07時10分28秒 JST -->
...
`cortex-m`フレームワークでは`rp2040_hal`クレートが`sio::spinlock0-31`を提供している。
...
上述のように、Cortex-MはVTORによる設定で割り込みベクタをメモリ上の任意の場所に配置できる。しかし`boot2`はFlash上の`0x2000_0100`に割り込みベクタを配置することに注意が必要。
